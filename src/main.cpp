...
int main() {
	...
	if (FileExists("max-score.txt")) {
		std::string maxScore = LoadFileText("max-score.txt");
		std::vector<std::string> splitVec{};
		std::istringstream maxScoreStream{maxScore};
		std::string splitInstance;
		while (std::getline(maxScoreStream, splitInstance, '\n')) {
			maxScore = splitInstance;
			splitVec.push_back(splitInstance);
		}
		std::string final;
		for (auto const& str : splitVec) {
			if (!str.empty() && str[0] != '#') {
				final = str;
				break;
			}
		...
		try {
			Snake::s_maxScore = std::stoi(final);
		} catch (std::invalid_argument const&) {
			TraceLog(LOG_WARNING, "Error parsing Max Score from file.");
			TraceLog(LOG_WARNING, "Got:\n%s", maxScoreStream.str().c_str());
			TraceLog(LOG_WARNING, "Parsed:\n%s", maxScore.c_str());
		}
	}
	...
	if (!SaveFileText("max-score.txt", TextFormat(
		...
		"%i", Snake::s_maxScore
	))) TraceLog(LOG_ERROR, "Failed to save max score.");
