...
void Game::saveLogs() {
	...
	fs::path logFilepath = logsDir/TextFormat("%04d.%02d.%02d-%02d:%02d:%02d.log",
	...
	if (m_hadWarning) {
		std::cerr << "\033[1;31mThis is a placeholder.\n"
			"needs to be replaced with something that informs the user that\n"
			"there has been a warning";
		if (!saved) std::cerr << " and the logs failed to save";
		std::cerr << ".\033[0m\n";
	}
}
